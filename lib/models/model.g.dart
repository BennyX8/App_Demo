...
class TableProduct extends SqfEntityTableBase {
  TableProduct() {
    ...
    fields = [
      ...
      SqfEntityFieldBase('cost', DbType.numeric),
      SqfEntityFieldBase('price', DbType.numeric),
...
class MyAppDabModel extends SqfEntityModelProvider {
  MyAppDabModel() {
...
class Product extends TableBase {
  Product({this.id, this.name, this.cost, this.price, this.image}) {
    _setDefaultValues();
    softDeleteActivated = false;
  }
  Product.withFields(this.name, this.cost, this.price, this.image) {
    _setDefaultValues();
  }
  Product.withId(this.id, this.name, this.cost, this.price, this.image) {
    _setDefaultValues();
  }
  // fromMap v2.0
  Product.fromMap(Map<String, dynamic> o, {bool setDefaultValues = true}) {
    ...
    if (o['cost'] != null) {
      cost = int.tryParse(o['cost'].toString());
    }
    if (o['price'] != null) {
  ...
  @override
  List<dynamic> toArgs() {
    return [name, cost, price, image];
  }

  @override
  List<dynamic> toArgsWithIds() {
    return [id, name, cost, price, image];
  }
  ...
  static Future<List<dynamic>> saveAll(List<Product> products,
    ...
    final isStartedBatch = await MyAppDabModel().batchStart();
    for (final obj in products) {
      await obj.save(ignoreBatch: false);
    }
    if (!isStartedBatch) {
      result = await MyAppDabModel().batchCommit(
          exclusive: exclusive,
          noResult: noResult,
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnProduct.rawInsert(
          'INSERT OR REPLACE INTO products (id, name, cost, price, image)  VALUES (?,?,?,?,?)',
          [id, name, cost, price, image],
          ignoreBatch);
      if (result! > 0) {
...
class ProductFilterBuilder extends ConjunctionBase {
  ...
  ProductField get cost {
    return _cost = _setField(_cost, 'cost', DbType.numeric);
  }

  ProductField? _price;
  ProductField get price {
    return _price = _setField(_price, 'price', DbType.numeric);
  }

  ProductField? _image;
  ProductField get image {
...
class ProductFields {
  ...
  static TableField get cost {
    return _fCost =
        _fCost ?? SqlSyntax.setField(_fCost, 'cost', DbType.numeric);
  }

  static TableField? _fPrice;
  static TableField get price {
    return _fPrice =
        _fPrice ?? SqlSyntax.setField(_fPrice, 'price', DbType.numeric);
  }
...
class IdentitySequence {
  ...
  Future<int> nextVal([VoidCallback Function(int o)? nextval]) async {
    final val = await MyAppDabModelSequenceManager()
...
class MyAppDabModelSequenceManager extends SqfEntityProvider {
  MyAppDabModelSequenceManager() : super(MyAppDabModel());
}
// END OF ENTITIES
